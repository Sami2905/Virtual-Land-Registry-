      ...
      <ul className="timeline-list">
        {lands.map((land) => (
          <li key={land.id.toString()} className="timeline-item">
            ...
            <span className="timeline-title">{land.name || `Land #${land.id}`}</span>
...
function MyLands({ principal, backend }) {
  ...
  useEffect(() => {
    ...
    // GSAP for card grid
    animateOnScroll('.mylands-card', {
      from: { opacity: 0, scale: 0.92, y: 40 },
      to: { opacity: 1, scale: 1, y: 0, stagger: 0.10, duration: 1, ease: 'power3.out' }
    }, { start: 'top 85%' });
        ...
        <AnimatePresence>
          {!loading && lands.length === 0 ? (
            <motion.p
              ...
              transition={{ duration: 0.5 }}
            >
              You haven't minted any lands yet. Head over to "Mint Land" to create your first plot!
            </motion.p>
          ) : (
            <motion.ul
              className="land-list"
              ...
              style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', listStyle: 'none', padding: 0 }}
            >
              <AnimatePresence>
                {lands.map((land, i) => (
                  <motion.li
                    ...
                    whileHover={{ scale: 1.04, boxShadow: '0 6px 18px rgba(0,0,0,0.18)' }}
                    whileTap={{ scale: 0.97 }}
                    style={{
                      background: '#121212',
                      borderRadius: 12,
                    ...
                    tabIndex={0}
                    aria-label={`Owned land card for ${land.name || land.id.toString()}`}
                  >
                    <img
                      src={land.image_data && land.image_data.length > 0 ? toBase64(land.image_data) : PLACEHOLDER_IMG}
                      alt={`Land: ${land.name || land.id.toString()}`}
                      style={{ width: '100%', aspectRatio: '4/3', objectFit: 'cover', borderRadius: 10, marginBottom: '1rem' }}
                    />
                    <motion.div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', marginBottom: '0.5rem' }} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                      <span style={{ background: '#0066ff', color: '#fff', borderRadius: 999, padding: '0.25rem 0.75rem', fontWeight: 600, fontSize: '0.98rem' }}>Owned</span>
                    </motion.div>
                    <div style={{ fontWeight: 600, fontSize: '1.2rem', color: '#fff', marginBottom: '0.3rem' }}>{land.name || `Land #${land.id.toString()}`}</div>
                    <div style={{ color: '#A0A0A0', fontSize: '1rem', marginBottom: '0.3rem' }}>
