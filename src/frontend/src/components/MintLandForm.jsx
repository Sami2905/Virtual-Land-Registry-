...
function MintLandForm({ principal, backend }) {
  ...
  useEffect(() => {
    ...
    // GSAP for stepper
    animateOnScroll('.mintland-stepper', {
      from: { opacity: 0, x: -40 },
      to: { opacity: 1, x: 0, stagger: 0.08, duration: 0.9, ease: 'power2.out' }
    }, { start: 'top 85%' });
    // GSAP for form
    animateOnScroll('.mintland-form', {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%' }}>
        <h2 className="mintland-title" style={{ fontSize: '2.2rem', fontWeight: 800, marginBottom: '2rem', color: '#fff', letterSpacing: '0.01em' }}>🪙 Mint Land</h2>
        <motion.form
        ...
        >
          {/* Stepper UI */}
          <div className="tx-stepper mintland-stepper" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', marginBottom: '2rem', gap: '1.2rem' }}>
            {steps.map((step, idx) => (
              <motion.div
                key={step.label}
              ...
              >
                <motion.div
                  style={{
                    ...
                    fontWeight: 700,
                    fontSize: '1.1rem',
                    border: currentStep === idx ? '2px solid #0066ff' : '2px solid #23233a',
          ...
          {[{
            ...
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.15 + i * 0.08, duration: 0.4, type: "spring", stiffness: 80 }}
              style={{ marginBottom: '1.2rem' }}
            >
              <label htmlFor={field.id} style={{ fontSize: '1rem', color: '#A0A0A0', fontWeight: 500, marginBottom: 6 }}>{field.label}</label>
              <Input
                id={field.id}
                value={field.value}
          ...
          <motion.div
            ...
            initial={{ opacity: 0, x: -30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.47, duration: 0.4, type: "spring", stiffness: 80 }}
            style={{ marginBottom: '1.2rem' }}
          >
            <label htmlFor="landImage" style={{ fontSize: '1rem', color: '#A0A0A0', fontWeight: 500, marginBottom: 6 }}>Land Image:</label>
            <Input
              key={fileInputKey}
              id="landImage"
              type="file"
