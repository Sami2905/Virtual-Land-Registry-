...
function GlowButton({ children, onClick, style = {}, ...props }) {
  return (
    <motion.button
      whileHover={{ scale: 1.08, boxShadow: "0 0 32px 12px #3b82f6, 0 2px 16px #a21caf55", background: "linear-gradient(90deg, #3b82f6 0%, #60a5fa 100%)" }}
      whileTap={{ scale: 0.96 }}
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        transform: `rotateY(${tilt.x * 8}deg) rotateX(${-tilt.y * 8}deg) scale(1.03)`,
        transition: "transform 0.18s cubic-bezier(.4,0,.2,1)",
        ...style
      }}
      whileHover={{ scale: 1.06, boxShadow: "0 0 48px #3b82f6cc, 0 2px 24px #a21caf55" }}
      {...props}
    >
      {children}
    </motion.div>
...
function Marketplace({ backend, principal }) {
  ...
  useEffect(() => {
    ...
    animateOnScroll('.marketplace-unlisted-card', {
      from: { opacity: 0, scale: 0.92, y: 40 },
      to: { opacity: 1, scale: 1, y: 0, stagger: 0.12, duration: 1, ease: 'power3.out' }
    }, { start: 'top 85%' });
    // GSAP for marketplace listings cards
    animateOnScroll('.marketplace-listing-card', {
      from: { opacity: 0, scale: 0.92, y: 40 },
      to: { opacity: 1, scale: 1, y: 0, stagger: 0.12, duration: 1, ease: 'power3.out' }
    }, { start: 'top 85%' });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 className="marketplace-title" style={{ fontSize: '2.4rem', fontWeight: 800, marginBottom: '1.5rem', letterSpacing: '0.01em', color: '#fff' }}>🏪 Marketplace</h2>
        {status && (
          <p className="marketplace-status" style={{ color: status.includes("✅") ? "#22c55e" : "#ef4444", fontWeight: 600, marginBottom: '1.5rem' }}>{status}</p>
        )}
        {loading ? (
          ...
          <>
            ...
            {myUnlistedLands.length > 0 && (
              <div style={{
                ...
                <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#fff', marginBottom: '1.2rem', letterSpacing: '0.01em', fontFamily: 'Inter, DM Sans, sans-serif' }}>🏠 Your Lands (Available to List)</h3>
                <ul style={{ listStyle: "none", padding: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '2.2rem', justifyContent: 'center' }}>
                  <AnimatePresence>
                    {myUnlistedLands.map((land, i) => (
                      ...
                      >
                        <img
                          src={land.image_data && land.image_data.length > 0 ? toBase64(land.image_data) : PLACEHOLDER_IMG}
                          alt={land.name || "Land"}
                          style={{ width: '100%', aspectRatio: '4/3', objectFit: 'cover', borderRadius: 16, marginBottom: '1rem', boxShadow: '0 2px 12px #3b82f655' }}
                        />
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', marginBottom: '0.5rem' }}>
                          <span style={{ background: 'var(--accent-blue)', color: '#fff', borderRadius: 999, padding: '0.25rem 0.95rem', fontWeight: 700, fontSize: '1.01rem', boxShadow: '0 1px 8px #3b82f655', fontFamily: 'Inter, DM Sans, sans-serif' }}>Available</span>
                        </div>
                        <div style={{ fontWeight: 800, fontSize: '1.22rem', color: '#fff', marginBottom: '0.3rem', letterSpacing: '-0.01em', fontFamily: 'Inter, DM Sans, sans-serif' }}>{land.name}</div>
                        <div style={{ color: '#A0A0A0', fontSize: '1rem', marginBottom: '0.3rem', fontFamily: 'Inter, DM Sans, sans-serif' }}>
                          <span>ID: {land.id.toString()}</span> &nbsp;|&nbsp;
                          <span>Size: {land.size}</span> &nbsp;|&nbsp;
                          <span>Coords: ({land.coordinates.x}, {land.coordinates.y})</span>
                        </div>
                        <motion.button
            ...
            <div className="landing-section" style={{ paddingTop: 0 }}>
              <h2 className="section-header" style={{ textAlign: 'center', marginBottom: 48 }}>Marketplace</h2>
              <div className="features-grid" style={{ flexWrap: 'wrap', gap: '2.5rem', justifyContent: 'center' }}>
                <AnimatePresence>
                  {allListings.length === 0 ? (
                    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} style={{ color: '#A0A0A0', fontWeight: 500, fontSize: 22, textAlign: 'center', width: '100%' }}>
                    ...
                    allListings.map(([land, listing], i) => (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 40 }}
                        transition={{ delay: i * 0.08, duration: 0.5, type: "spring", stiffness: 80 }}
                        style={{ display: 'flex', justifyContent: 'center', width: '100%', maxWidth: 360 }}
                      >
                        <TiltCard
                          tabIndex={0}
                        ...
                        >
                          <div style={{ padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            <FloatingIcon style={{ marginBottom: 8 }}><IconExplore style={{ width: 48, height: 48 }} /></FloatingIcon>
                            <img
                              src={land.image_data && land.image_data.length > 0 ? toBase64(land.image_data) : PLACEHOLDER_IMG}
                              alt={land.name || "Land"}
                              style={{ width: 180, height: 120, borderRadius: 12, objectFit: 'cover', marginBottom: 18, background: '#181a22', boxShadow: '0 2px 12px #3b82f655' }}
                            />
                            <div style={{ fontWeight: 800, fontSize: 22, color: "#fff", marginBottom: 6, letterSpacing: "-0.01em" }}>{land.name}</div>
                            <div style={{ color: '#A0A0A0', fontSize: 15, marginBottom: 8 }}>ID: {land.id.toString()} | Size: {land.size}</div>
                            <div style={{ color: '#A0A0A0', fontSize: 15, marginBottom: 8 }}>Coords: ({land.coordinates.x}, {land.coordinates.y})</div>
                            <div style={{ color: '#fff', fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Price: <span style={{ color: '#3b82f6', fontWeight: 800 }}>{listing.price} ICP</span></div>
                            <GlowButton style={{ fontSize: 15, padding: '0.9rem 2.2rem', marginTop: 8 }} onClick={() => handleBuy(land.id)}>Buy Now</GlowButton>
                          </div>
                        </TiltCard>
                      </motion.div>
                    ))
